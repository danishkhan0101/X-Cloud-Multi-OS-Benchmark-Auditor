...
while IFS=$'\t' read -r raw_name raw_ip raw_os raw_power raw_offer; do
    ...
    if [[ "$os" == *"Linux"* ]] || [[ "$os" == *"Ubuntu"* ]]; then
        if [[ "$offer" == *"rocky"* ]] || [[ "${vm_name,,}" == *"rocky"* ]]; then
            DISTRO="Rocky"; LINUX_USER="$GHOST_USER"
        elif [[ "$offer" == *"rhel"* ]] || [[ "${vm_name,,}" == *"rhel"* ]]; then
            DISTRO="RHEL"; LINUX_USER="$GHOST_USER"   
        else
            DISTRO="Ubuntu"; LINUX_USER="$UBUNTU_USER"
        fi

        # 🛡️ FIX 1: Bulletproof SSH Handshake Validation
        SSH_CHECK=$(ssh -n -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no ${LINUX_USER}@${ip} "echo SSH_OK" 2>/dev/null)
        ...
        if [ "$DISTRO" == "Rocky" ]; then ROCKY_MACHINES+=("$ip"); echo -e "${CYAN}🏔️ Mapped Rocky Node: $ip${NC}"
        elif [ "$DISTRO" == "RHEL" ]; then RHEL_MACHINES+=("$ip"); echo -e "${CYAN}🔴 Mapped RHEL Node: $ip${NC}"
        ...
        sleep 20
        WINDOWS_MACHINES+=("$ip")
        echo -e "${CYAN}🪟 Mapped Windows Node: $ip${NC}"
    fi
done <<< "$VM_DATA"

# ======================================================
# 🚨 GITHUB ANNOTATION EXIT
# ======================================================
...
run_phase_1() {
    ...
    if [ "$H_TARGET_OS" == "all" ] || [ "${H_TARGET_OS,,}" == "ubuntu" ]; then
        for IP in "${UBUNTU_MACHINES[@]}"; do
            ...
            # 🚨 RESTORED: Ubuntu SCAP Fallback Injector
...
run_phase_4() {
    ...
    if [ "$H_TARGET_OS" == "all" ] || [ "${H_TARGET_OS,,}" == "ubuntu" ]; then
        for IP in "${UBUNTU_MACHINES[@]}"; do
            ...
            if [ "$RUN_CIS" == true ]; then
                echo -e "${GREEN}✅ [UBUNTU - CIS L${OS_LVL}] Verifying $IP...${NC}"
            ...
            if [ "$RUN_ORG" == true ]; then
                echo -e "${GREEN}✅ [UBUNTU - $ORG_NAME] Verifying $IP...${NC}"
                scp -o BatchMode=yes -o StrictHostKeyChecking=no "$UBUNTU_CUSTOM_OVAL" "$UBUNTU_CUSTOM_XCCDF" ${UBUNTU_USER}@${IP}:/tmp/ > /dev/null 2>&1 || true
...
run_cleanup() {
    ...
    if [ "$H_TARGET_OS" == "all" ] || [ "${H_TARGET_OS,,}" == "rhel" ]; then
        for IP in "${RHEL_MACHINES[@]}"; do
            ...
            # 🛡️ FIX 3: Permanently delete the Ghost User from RHEL
            VM_NAME=$(az vm list-ip-addresses -g "$RG_NAME" --query "[?virtualMachine.network.publicIpAddresses[0].ipAddress=='$IP'].virtualMachine.name" -o tsv)
            if [ -n "$VM_NAME" ]; then
                az vm run-command invoke -g "$RG_NAME" -n "$VM_NAME" --command-id RunShellScript --scripts "userdel -r ${GHOST_USER}" -o none > /dev/null 2>&1 || true
            fi
        done
    fi
    ...
    if [ "$H_TARGET_OS" == "all" ] || [ "${H_TARGET_OS,,}" == "rocky" ]; then
        for IP in "${ROCKY_MACHINES[@]}"; do
            ...
            # 🛡️ FIX 3: Permanently delete the Ghost User from Rocky
            VM_NAME=$(az vm list-ip-addresses -g "$RG_NAME" --query "[?virtualMachine.network.publicIpAddresses[0].ipAddress=='$IP'].virtualMachine.name" -o tsv)
            if [ -n "$VM_NAME" ]; then
                az vm run-command invoke -g "$RG_NAME" -n "$VM_NAME" --command-id RunShellScript --scripts "userdel -r ${GHOST_USER}" -o none > /dev/null 2>&1 || true
            fi
